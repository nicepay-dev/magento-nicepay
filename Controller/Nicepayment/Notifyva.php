...
class Notifyva extends AbstractNotification
{
    ...
    public function __construct(
        ...
        $this->logPrefix = uniqid() . ' : NicePayment Ewallet Snap Notification - ';
    }

    /**
     * Notification API for Virtual Account
     *
     */
    public function execute()
    {
        ...
        try {
            ...
            $referenceNo = $incomingNotif['trxId'] ?? "";
            $partnerServiceId = $incomingNotif['partnerServiceId'] ?? "";
            $customerNo = $incomingNotif['customerNo'] ?? "";
            $vaNo = $incomingNotif['virtualAccountNo'] ?? "";
            $amount = $incomingNotif['paidAmount']['value'] ?? "";
            $paymentRequestId = $incomingNotif['paymentRequestId'] ?? "";
            ...
            if (!$order) {
                $niceLogger->info($this->logPrefix . 'Order not found');
            ...
            if ($isVerified && !$isOrderAlreadyPaid) {
                ...
                $nicepay->set('vacctNo', $vaNo);
                $nicepay->set('customerNo', $customerNo);
                $nicepay->set('partnerServiceId', $partnerServiceId);
                    ...
                    if ($transactionStatus == '00' && $order->getState() == Order::STATE_PENDING_PAYMENT) {
                        ...
                        $responseData = [
                            ...
                            'virtualAccountData' => [
                                ...
                                'virtualAccountName' => $order->getCustomerName(),
                                'trxId' => $referenceNo,
                                ...
                                'trxDateTime' => Helper::getFormattedTimestampV2()
                            ],
                            'additionalInfo' => [
                                'bankCd' => $responseInqStatus['bank_code'] ?? '',
                                'goodsNm' => "",
                            ]
                        ];
                    } else {
                ...
                $niceLogger->info("Order transaction already paid");
                $responseCd = "400";
                $responseData = [
