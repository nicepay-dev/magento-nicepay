...
class Reject extends AbstractAction
{
    ...
    public function __construct(
        ...
        $this->logPrefix = uniqid() . ' NicePayout Menu : Reject - ';
    }
    ...
    public function execute()
    {
        ...
        $niceLogger->info($this->logPrefix . 'Reject execute start ');
        ...
        try {
            ...
            $niceLogger->info($this->logPrefix . 'Reject Response ', ['response' => $nicepayResponse]);

            if ($nicepayResponse['status_code'] == "2000000" || $nicepayResponse['status_code'] == "0000") {
                $niceLogger->info($this->logPrefix . 'Request Reject Payout Transaction to Nicepay success');
                $this->messageManager->addSuccessMessage(__('Transacrtion %1 has been rejected.', $id));
                ...
                $niceLogger->info($this->logPrefix . 'Inquiry Response ' . json_encode($nicepayInquiryResponse));
                ...
                if (($nicepayInquiryResponse['status_code'] == "2000000" || $nicepayInquiryResponse['status_code'] == "0000") && !empty($nicepayInquiryResponse['status_trx'] ?? null)) {
                    $niceLogger->info($this->logPrefix . 'Update Status to : ' . $nicepayInquiryResponse['status_trx']);
                    $order->setNicepayPaymentStatus($nicepayInquiryResponse['status_trx']);
                    $this->getOrderRepo()->save($order);
                }
            } else {
                $niceLogger->error($this->logPrefix . 'Request Reject Payout Transaction to Nicepay failed');
                $this->messageManager->addErrorMessage(__('Transacrtion %1 Rejection Failed : %2', $id, $nicepayResponse['status']));
            }
        } catch (Exception $e) {
            $niceLogger->error($this->logPrefix . 'Reject execute Exception : ' . $e->getMessage());
