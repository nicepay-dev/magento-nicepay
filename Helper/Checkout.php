    ...
    public function processOrdersFailedPayment(array $orderIds, string $failureReason = 'Unexpected Error with empty charge')
    {
        foreach ($orderIds as $orderId) {
            ...
            $order->setState($orderState)
                ...
                ->addCommentToStatusHistory("Order #" . $orderId . " was rejected by Nicepay because " . $failureReason);
    ...
    protected function getOrderItemsQty(Order $order): array
    {
        ...
        foreach ($order->getAllVisibleItems() as $orderItem) {
            ...
            if (!$orderItemProduct->isSalable()) {
                throw new \Magento\Framework\Exception\LocalizedException(
                    __(
                        'Payment processed on Nicepay (transaction_id %1). Order failed due to insufficient inventory',
                        $order->getData('nicepay_txid')
                    )
                );
            }
            ...
            foreach ($orderItem->getChildrenItems() as $childItem) {
                ...
                if (!$childItemProduct->isSalable()) {
                    throw new \Magento\Framework\Exception\LocalizedException(
                        __(
                            'Payment processed on Nicepay (txid %1). Order failed due to insufficient inventory',
                            $order->getData('nicepay_txid')
                        )
                    );
                }
